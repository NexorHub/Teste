...
-- Variável de módulo para a cor de destaque atual (usada pelo CircleClick)
local _currentAccentColor = Color3.fromRGB(255, 255, 255)
...
function CircleClick(Button, X, Y)
    spawn(function()
        ...
        Circle.ImageColor3 = _currentAccentColor
...
--- Alias usado pelo KZ.lua: BastardXHub:Notification({Title, Content, SubTitle, Duration, Color})
function BastardXHub:Notification(cfg)
    cfg = cfg or {}
    return self:MakeNotify({
        Title       = cfg.Title    or "KAIKEN HUB",
        Description = cfg.SubTitle or "",
        Content     = cfg.Content  or "",
        Color       = cfg.Color    or Color3.fromRGB(255, 255, 255),
        Delay       = cfg.Duration or 3,
        Time        = 0.5,
    })
end
...
function BastardXHub:Window(GuiConfig)
    ...
    -- ── Verificação exclusiva: o título precisa começar com "KAIKEN" ──
    local rawTitle = tostring(GuiConfig.Title or "")
    if not rawTitle:upper():match("^KAIKEN") then
        error("[KAIKEN LIB] ❌ Esta biblioteca é exclusiva do KAIKEN HUB.")
        return {}
    end

    GuiConfig.Title        = GuiConfig.Title or "KAIKEN LIB"
    -- Injeta o emoji 💢 depois de "KAIKEN HUB" no título (feito aqui pois emojis bugam em executors)
    GuiConfig.Title        = GuiConfig.Title:gsub("^(KAIKEN HUB)", "%1 💢")
    ...
    _currentAccentColor = GuiConfig.Color   -- inicializa CircleClick com a cor do hub
    ...
    -- registra para SetAccentColor atualizar em tempo real
    Tabs._accentRefs.dropStrokes = Tabs._accentRefs.dropStrokes or {}
    table.insert(Tabs._accentRefs.dropStrokes, UIStroke14)

    DropdownSelectReal.AnchorPoint = Vector2.new(0.5, 0.5)
    DropdownSelectReal.BackgroundColor3 = GuiConfig.Color
    ...
    -- registra para SetAccentColor atualizar em tempo real
    Tabs._accentRefs.dropBgs = Tabs._accentRefs.dropBgs or {}
    table.insert(Tabs._accentRefs.dropBgs, DropdownSelectReal)
    ...
    Tabs._accentRefs = {
        ...
        dropStrokes  = {},   -- UIStroke14 (dropdown border)
        dropBgs      = {},   -- DropdownSelectReal (dropdown background)
    ...
    function Tabs:SetAccentColor(color)
        ...
        _currentAccentColor = color   -- atualiza CircleClick
        ...
        -- Borda do painel dropdown (UIStroke14)
        for _, ds in ipairs(self._accentRefs.dropStrokes or {}) do
            if ds and ds.Parent then
                TweenService:Create(ds, TweenInfo.new(0.3), { Color = color }):Play()
            end
        end
        -- Fundo do painel dropdown (DropdownSelectReal)
        for _, db in ipairs(self._accentRefs.dropBgs or {}) do
            if db and db.Parent then
                TweenService:Create(db, TweenInfo.new(0.3), { BackgroundColor3 = color }):Play()
            end
        end
    ...
    --- Registra o botão flutuante na janela.
    function Tabs:RegisterFloat(floatFunc)
        self._float = floatFunc
    end

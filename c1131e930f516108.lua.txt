...
function BastardXHub:Window(GuiConfig)
    ...
    Tabs._accentRefs = {
        ...
        dropdownStroke = UIStroke14,        -- Borda do painel de dropdown
        dropdownReal   = DropdownSelectReal, -- Fundo colorido do painel de dropdown
    }
    local CountTab = 0
    ...
    function Tabs:SetAccentColor(color)
        ...
        -- Painel de dropdown (borda + fundo colorido)
        if self._accentRefs.dropdownStroke and self._accentRefs.dropdownStroke.Parent then
            TweenService:Create(self._accentRefs.dropdownStroke, TweenInfo.new(0.3), { Color = color }):Play()
        end
        if self._accentRefs.dropdownReal and self._accentRefs.dropdownReal.Parent then
            TweenService:Create(self._accentRefs.dropdownReal, TweenInfo.new(0.3), { BackgroundColor3 = color }):Play()
        end
    end

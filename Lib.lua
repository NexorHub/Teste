--[[
    Bastard X Hub  -  Biblioteca de Interface para Roblox
    Versao : 2.0.0
    Icones : github.com/NexorHub/Teste  (carregados via HTTP)
    Temas  : Ciano / Roxo / Verde / Vermelho / Rosa / Dourado

    USO RAPIDO:
        local BXH = loadstring(game:HttpGet("SUA_URL"))()
        BXH:SetToggleIcon("rbxassetid://104112878732002")
        local Win = BXH:CreateWindow({
            Title = "Meu Hub",
            Icon  = "sword",
            Intro = { Title = "Script", Subtitle = "v1.0 by dev", Duration = 3 }
        })
        local Tab = Win:CreateTab({ Name = "Farm", Icon = "axe" })
        local Sec = Tab:CreateSection("Auto Farm")
        Sec:CreateButton({ Name = "Iniciar", Callback = function() end })
        Sec:CreateToggle({ Name = "Auto Farm", Default = false, Callback = function(v) end })
        Sec:CreateSlider({ Name = "Velocidade", Min=0, Max=200, Default=16, Callback=function(v) end })
        Sec:CreateDropdown({ Name = "Mob", Options={"Boss","Mob1"}, Callback=function(v) end })
        Win:Notify({ Title="Pronto", Content="Carregado!", Type="Success" })
]]
...
-- ═══════════════════════════════════════════════════════════════
--  ADAPTADOR DE TELA
--  Ajusta tamanhos e espacamentos conforme a resolucao da tela.
-- ═══════════════════════════════════════════════════════════════
local SA = {}
do
...
-- ═══════════════════════════════════════════════════════════════
--  NOMES SEGUROS
--  Nomes de instancia com unicode/emoji travam alguns executors.
--  Usa contador monotono - garante nomes unicos e seguros (ASCII).
-- ═══════════════════════════════════════════════════════════════
local _N = 0
...
-- ═══════════════════════════════════════════════════════════════
--  PALETAS DE CORES
--  Ciano | Vermelho | Roxo | Verde | Dourado | Rosa
-- ═══════════════════════════════════════════════════════════════
local PALETTES = {
    Cyan = {
        Cyan  = Color3.fromHex("00FFFF"),
...
--   Callback : { fn }              - fn(pal) decide o que fazer (ex: tab ativa/inativa)
local _AC = {}
...
-- Persistencia de tema entre sessoes (salvo em arquivo)
local THEME_FILE = "BastardXHub_theme.txt"
...
-- Aplica nova paleta de cores em tempo real com animacao suave
...
-- Carrega tema salvo ao iniciar (noSave=true para nao re-salvar)
...
--  Carrega todos os icones do repositorio via HTTP.
--  Icons.Apply() aplica imediatamente se ja carregados,
--  ou enfileira para aplicar apos o carregamento terminar.
-- ═══════════════════════════════════════════════════════════════
local Icons = {}
Icons._cache = nil; Icons._ready = false; Icons._queue = {}
Icons.URL = "https://raw.githubusercontent.com/NexorHub/Teste/refs/heads/main/Icons.lua"
...
task.spawn(function()
    ...
    Icons._ready = true
    if not ok then warn("[BastardXHub] Falha ao carregar icones: "..tostring(res)) end
...
--  RC=corner, SK=stroke, PD=padding, LH=layout-h, LV=layout-v
--  Fr=frame, Lb=label, IL=imageLabel, IB=imageButton, TB=textButton
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
--  CAMADA DE OVERLAY
--  Referencia ao ScreenGui principal para elementos que precisam
--  ficar acima de qualquer ClipsDescendants (ex: config panel).
-- ═══════════════════════════════════════════════════════════════
...
--  Usa StarterGui:SetCore("SendNotification") - notificacao nativa.
--  Suporta: Title, Content, Duration, Icon (rbxassetid:// ou nome)
-- ═══════════════════════════════════════════════════════════════
local Notif = {}
local StarterGui = game:GetService("StarterGui")

function Notif.Init(_sg) end  -- mantido para compatibilidade futura

function Notif.Send(o)
    o = o or {}
    -- StarterGui:SetCore pode nao estar disponivel em alguns contextos
    if not pcall(function() return game:GetService("StarterGui") end) then return end
    local iconId = o.Icon
    -- Resolve o nome do icone via repositorio (ex: "shield", "bell")
    -- Aceita numero direto ou string "rbxassetid://..."
...
-- ═══════════════════════════════════════════════════════════════
--  MIXIN DE ARRASTAR
--  Torna qualquer janela arrastavel pelo cabecalho.
-- ═══════════════════════════════════════════════════════════════
...
--  Cada funcao retorna (wrap, api).
--  wrap = frame raiz do componente (ja adicionado ao parent)
--  api  = tabela com metodos Set/Get/etc.
-- ═══════════════════════════════════════════════════════════════
local CF={}

-- ─── BOTAO ─────────────────────────────────────────────────────
-- ═══════════════════════════════════════════════════════════════
--  SISTEMA DE SALVAMENTO AUTOMATICO
--  Salva o estado de Toggles, Sliders e Dropdowns automaticamente.
--  Uso: BXH:EnableAutoSave()          -> salva em "BastardXHub_autosave.txt"
--  Personalizado: BXH:EnableAutoSave("MeuHub") -> "MeuHub_autosave.txt"
-- ═══════════════════════════════════════════════════════════════
local _AS = {
    enabled  = false,
    file     = "BastardXHub_autosave.txt",
    data     = {},   -- { [key] = value }
    dirty    = false,
}

local function AS_key(sectionName, elemName)
    -- chave unica: "NomeSecao||NomeElemento"
...
function CF.Button(o,parent,z)
    ...
    hit.MouseButton1Click:Connect(function()
        ...
        if o.Callback then
            task.spawn(function() local ok,err=pcall(o.Callback); if not ok then warn("[BastardXHub Btn]",err) end end)
        end
    end)
    return wrap
end
...
function CF.Toggle(o,parent,z)
    ...
    -- AutoSave: recupera estado salvo antes de criar os elementos visuais
    do
    ...
    local function Set(v,fire)
        ...
        if fire and o.Callback then task.spawn(function() local ok,err=pcall(o.Callback,state); if not ok then warn("[BXH Toggle]",err) end end) end
    end
    local hit=TB(TH.Bg,1,z+10); hit.Size=UDim2.fromScale(1,1); hit.Parent=wrap
...
function CF.Slider(o,parent,z)
    ...
    -- AutoSave: recupera valor salvo antes de criar os elementos visuais
    do
        local sv = AS_get(o._secName, o.Name)
    ...
    local function SetVal(v,fire)
        ...
        if fire and o.Callback then task.spawn(function() local ok,err=pcall(o.Callback,val); if not ok then warn("[BXH Slider]",err) end end) end
    end
...
-- ─── DROPDOWN ──────────────────────────────────────────────────
-- O painel de opcoes e filho do proprio wrap (inline).
-- Ao abrir, o wrap cresce e empurra os elementos abaixo via UIListLayout.
-- Ao fechar, o wrap volta ao tamanho original.
function CF.Dropdown(o,parent,z)
    o=o or{}; z=z or 5
    local options=o.Options or{}
    local selected=o.Default or(options[1] or"Select...")
    -- AutoSave: recupera opcao salva (se ainda existir nas opcoes atuais)
    do
        local sv = AS_get(o._secName, o.Name)
        if sv ~= nil then
    ...
    -- Wrap: tamanho fixo inicial (apenas o cabecalho).
    -- Ao abrir, cresce via animacao para empurrar os elementos abaixo no UIListLayout pai.
    ...
    -- Cabecalho: altura fixa, sempre visivel
    ...
    -- Painel inline: filho do wrap, aparece logo abaixo do cabecalho
    ...
    local function BuildItems()
        ...
        for _,opt in ipairs(options) do
            ...
            rb.MouseButton1Click:Connect(function()
                ...
                -- Fecha e encolhe o wrap para liberar espaco abaixo
                isOpen=false
                Anim.T(panel,{Size=UDim2.new(1,0,0,0)},TH.Fast)
                Anim.T(wrap,{Size=UDim2.new(1,0,0,SA.BtnH)},TH.Fast)
                ...
                if o.Callback then task.spawn(function() local ok,err=pcall(o.Callback,selected); if not ok then warn("[BXH Dropdown]",err) end end) end
            end)
        end
    end
    BuildItems()
    ...
    local function Open()
        ...
        -- Cresce o painel e o wrap juntos: o wrap maior empurra os elementos abaixo
        Anim.T(panel,{Size=UDim2.new(1,0,0,fullH)},TH.Elastic)
        Anim.T(wrap,{Size=UDim2.new(1,0,0,SA.BtnH+fullH+6)},TH.Elastic)
    ...
    local function Close()
        isOpen=false
        -- Encolhe o painel e o wrap: libera o espaco que os elementos abaixo ocupavam
        Anim.T(panel,{Size=UDim2.new(1,0,0,0)},TH.Fast)
        Anim.T(wrap,{Size=UDim2.new(1,0,0,SA.BtnH)},TH.Fast)
        task.delay(0.12,function() panel.Visible=false end)
        Anim.T(arrow,{Rotation=0},TH.Fast)
...
function CF.Keybind(o,parent,z)
    ...
    UserInputService.InputBegan:Connect(function(i,gp)
        ...
        if i.UserInputType==Enum.UserInputType.Keyboard then
            ...
            if o.Callback then task.spawn(function() local ok,err=pcall(o.Callback,key); if not ok then warn("[BXH Keybind]",err) end end) end
        end
    end)
...
function CF.Input(o,parent,z)
    ...
    tb.FocusLost:Connect(function(enter)
        ...
        Anim.T(wrap,{BackgroundColor3=TH.Surface},TH.Fast)
        if o.Callback then task.spawn(function() local ok,err=pcall(o.Callback,value,enter); if not ok then warn("[BXH Input]",err) end end) end
    end)
...
-- ═══════════════════════════════════════════════════════════════
--  BOTAO FLUTUANTE DE ALTERNANCIA
--  Botao arrastavel que mostra/oculta a janela principal.
-- ═══════════════════════════════════════════════════════════════
local function FloatBtn(sg,iconNameOrId,onToggle)
    local S = 46

    -- Anel de pulso: irmao do botao no ScreenGui (ZIndex menor = fica atras)
    -- Parentear ao sg diretamente garante circulo perfeito sem interferencia do pai.
    ...
    fb.Parent            = sg
    RC(fb, S / 2)
    -- PNG sem fundo: ImageColor3 branco para preservar cores originais da imagem
    fb.ImageColor3 = Color3.new(1, 1, 1)
    local fbStroke = SK(fb, TH.Cyan, 1.5, 0)
    RegAC(fbStroke, "Color", "Cyan")
    -- toggled inicia true; ao trocar de tema o fundo muda automaticamente
    local toggled = true
    RegACFn(function(pal)
        if fb and fb.Parent then
            -- Fundo troca com o tema: CyanD quando visivel, SurfaceB quando oculto
    ...
    -- Pulso animado: apenas tamanho e transparencia; forma nao muda (sempre circulo)
    task.spawn(function()
    ...
    -- Arrastar: move o botao E o anel juntos
    ...
    -- Clique: alterna visibilidade + micro-bounce no botao
...
-- ═══════════════════════════════════════════════════════════════
--  SISTEMA DE ABAS  (pills horizontais)
-- ═══════════════════════════════════════════════════════════════
local function BuildTabSystem(body,tabScroll,z)
    local tabs={}; local current=nil; local PILL_H=32
    -- Tamanho do icone adaptativo por quantidade de abas:
    -- ate 4 abas: 20px | ate 6: 17px | ate 9: 14px | mais de 9: 11px (minimo)
...
--  Painel flutuante: troca tema, transparencia e tamanho da janela.
-- ═══════════════════════════════════════════════════════════════
local function BuildConfigPanel(sg, root, winRef, z)
    ...
    -- Grade de amostras de cor: 3 por linha, 2 linhas (grade 3x2 para 6 temas)
    local PALETTE_ORDER = {"Cyan","Purple","Green","Red","Pink","Gold"}
    -- Cada celula: bolinha colorida + nome abaixo
    ...
    for idx, palName in ipairs(PALETTE_ORDER) do
        ...
        -- Celula: bolinha + nome empilhados verticalmente
        ...
        -- Nome do tema sempre visivel abaixo da bolinha
        ...
        -- Anel de selecao (filho do sw, atras do botao)
    ...
    -- Secao: controle de transparencia
    ...
    local transpVal = 0  -- intervalo: 0 a 80
    -- Slider compacto de transparencia
    ...
    local function TogglePanel()
        ...
        if panOpen then
            panel.Visible = true
            -- Calcula posicao absoluta com base no root da janela
            local vp = workspace.CurrentCamera.ViewportSize
            local rp = root.AbsolutePosition
            local rs = root.AbsoluteSize
            -- Posiciona proxximo ao botao de config (canto superior direito do root)
            local px = rp.X + rs.X - panW - 4
            local py = rp.Y + 50
            -- Garante que o painel nao saia da tela horizontalmente
            px = math.clamp(px, 4, vp.X - panW - 4)
            task.defer(function()
                local targetH = inner.AbsoluteSize.Y + 4
                -- Garante que nao saia da tela verticalmente
...
local function BuildWindow(opts,sg)
    opts=opts or{}
    local title=opts.Title or"BastardXHub"
    ...
    RC(root,10)
    -- Borda neon: UIStroke pulsante sincronizado com o CornerRadius do root
    ...
    -- ClipsDescendants=false e OBRIGATORIO: com true o Roblox recortaria o tbm fora da area curva.
    RC(titleBar,10)
    -- tbm: frame filho que preenche os 10px inferiores da titleBar,
    -- cobrindo os cantos arredondados inferiores -> borda reta embaixo.
...
local function BuildIntroScreen(sg, opts, onDone)
    ...
    -- Layout: posicoes fixas simples (sem UIListLayout para evitar conflito com topBar)
    ...
    -- Barra de progresso (fundo escuro)
    ...
    -- Define altura final do card com base no conteudo
    card.Size          = UDim2.fromOffset(cardW, Y)
    card.AutomaticSize = Enum.AutomaticSize.None

    -- Animacao de entrada do card
    ...
    task.spawn(function()
        ...
        task.wait(0.35)
        -- Abre a janela junto com o fade out
        if onDone then onDone() end
        -- Fade out de todos os elementos simultaneamente
...
function Library:_Init()
    ...
    local old=CoreGui:FindFirstChild("_BastardXHub"); if old then old:Destroy() end
    local sg=Instance.new("ScreenGui"); sg.Name="_BastardXHub"; sg.ResetOnSpawn=false
...
-- Define cores base manualmente (avancado)
...
-- Ativa o sistema de salvamento automatico de Toggles, Sliders e Dropdowns.
-- fileName (opcional): nome base do arquivo (ex: "MeuHub" -> "MeuHub_autosave.txt")
function Library:EnableAutoSave(fileName)
    _AS.file    = (fileName or "BastardXHub") .. "_autosave.txt"
    _AS.enabled = true
    AS_load()   -- carrega dados existentes imediatamente
end
-- Salva manualmente o estado atual (util para forcar um save pontual)
